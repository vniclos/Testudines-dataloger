...
///----------------------------------
String fncSD_FileChartsList(fs::FS &fs, const char *dirname, uint8_t levels)
{
  String szReply = "";
  String szFileName = "";
 ...
 szReply="<ul class='nobull'>";
  while (file)
  {
    if (file.isDirectory())
    {
      ...
      if (levels)
      {
        fncSD_FileChartsList(fs, file.name(), levels - 1);
      }
    }
    else
    {
      szFileName = file.name();
      if (szFileName.endsWith("dd.csv") || szFileName.endsWith("mm.csv") || szFileName.endsWith("yy.csv"))
      {
        /*
        Serial.print("  FILE: ");
...
bool fncSD_createDir(fs::FS &fs, const char *path)
{
...
bool fncSD_removeDir(fs::FS &fs, const char *path)
{
...
bool fncSD_writeFile(fs::FS &fs, const char *path, const char *message)
{
...
bool fncSD_appendFile(fs::FS &fs, const char *path, const char *message)
{
...
bool fncSD_renameFile(fs::FS &fs, const char *path1, const char *path2)
{
...
bool fncSD_deleteFile(fs::FS &fs, const char *path)
{
...
bool fncSD_testFileIO(fs::FS &fs, const char *path)
{
  File file = fs.open(path);
  static uint8_t buf[512];
  size_t len = 0;
