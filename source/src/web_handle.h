...
void handleChartsFileList()
{
  digitalWrite(PIN_LEDINT, 1);

  String szHtml = "\n<fieldset><legend>Select file to analize</legend><ol>\n";
  szHtml += fncSD_FileChartsList(SD_MMC, "/", 0);
  szHtml += "\n</ol></fieldset>";
...
void handleJSApexGetfile()
{
  ...
  else
  {
    ...
    if (szFileName == "/today")
    {
      ...
      sprintf(cFileName, "/%04d%02d%02ddd.csv", g_device_Rtc3231Now.Year(), g_device_Rtc3231Now.Month(), g_device_Rtc3231Now.Day());
      szFileName = String(cFileName);
    }
...
void handleReadSensors()
{
  ...
  fnc_devices_Read();
  String szHtml = "";
  szHtml += "<fieldset><legend>Dataloger Station</legend>";
  ...
  szHtml += "<br/><span class=\"fldtit\">Id. GPS coordinates :</span><span id=\"scnIdGps\" class=\"fldvalue\">" + String(g_cfg.IdLocationGps) + "</span> [Lat. ,Long.]";
  ...
  szHtml += "<br/><span class=\"fldtit\">Save interval day:</span><span class=\"fldvalue\">15 minutes Hour</span>";
...
void handlesetLocGPS()
{
  ...
  g_server.send(200, "text/html", String(g_Dev.s_GpsLatiLong));
  digitalWrite(PIN_LEDINT, 0);
}
...
void handleFileChartsList()
{
  String szCmd = "";
  String szfileName = "";
  String szReply = "<fieldset><legend>Charts files</legend><table>\n";
   szReply += "You can draw other charts by clicking on one of this files.";
  szReply += fncSD_FileChartsList(SD_MMC, "/", 0);
  szReply += "\n</table></fieldset>";
